# Demonswap Profile SDK

This package provides some handy functions to retrieve data for Demonswap Profile system.
...
Install `@demonswap/profile-sdk` into your project with npm:

```bash
npm install @demonswap/profile-sdk --save
```

or yarn:

```bash
yarn add @demonswap/profile-sdk
```
...
```js
import DemonProfileSdk from "@demonswap/profile-sdk";

const demonSdk = new DemonProfileSdk();
```
...
const demonSdk = new DemonProfileSdk(customProvider, 97);
```
...
const demonSdk = new DemonProfileSdk();
const username = demonSdk.getUsername("0x123456789");
...
It also sets `profile_${address}` cookie containing username and avatar (now only for demonswap.fi domain, maybe configurable in future versions)

```js
import DemonProfileSdk from "@demonswap/profile-sdk";

const demonSdk = new DemonProfileSdk();
const profile = demonSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = demonSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@demonswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [demon-frontend](https://github.com/demonswap/demon-frontend) repo. There are several improvements to be made in the future versions of this SDK:
