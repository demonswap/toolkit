...
it("renders correctly with unit prop", () => {
  const { asFragment } = renderWithTheme(
    <BalanceInput value="14" currencyValue="15 USD" unit="DEMON" onUserInput={handleChange} />
  );
  expect(asFragment()).toMatchInlineSnapshot(`
    <DocumentFragment>
      .c1 {
...
it("renders correctly with unit prop and switchEditingUnits", () => {
  ...
  expect(asFragment()).toMatchInlineSnapshot(`
    ...
    .c9:hover:not(:disabled):not(.demon-button--disabled):not(.demon-button--disabled):not(:active) {
      opacity: 0.65;
    }

    .c9:active:not(:disabled):not(.demon-button--disabled):not(.demon-button--disabled) {
      opacity: 0.85;
